        ...
        <div>
            <form class="form-horizontal" role="form">
                ...
                <div class="form-group">
                    <label for="uploadfile" class="col-sm-3 control-label">上传图片</label>
                    <div class="col-sm-7">
...
<script type="application/javascript">
    init();
    function init() {
        $('#update-houseId').text("${data.houseId}");
        $('#update-saleType').val("${data.saleType}");
        $('#update-houseType').val("${data.houseType}");
        $('#update-area').val("${data.area}");
        $('#update-price').val("${data.price}");
        $('#update-community').val("${data.communityName}");
        $('#update-address').val("${data.address}");
        $('#update-user').val("${data.authorName}");
        $('#update-tel').val("${data.authorTel}");
        $('#update-des').val("${data.des}");
        $('#fromcity').val("${data.city}");
    }

    function update(url) {
        var houseId = $('#update-houseId').text();
        var houseType = $('#update-houseType').val();
        var saleType = $('#update-saleType').val();
        var area = $('#update-area').val();
        var price = $('#update-price').val();
        var community = $('#update-community').val();
        var address = $('#update-address').val();
        var city = $('#fromcity').val();
        var authorTel = $('#update-tel').val();
        var user = $('#update-user').val();
        var des = $('#update-des').val();
        $.ajax({
            type: "POST",
            url: url + "/house/" + houseId + "/update",
            data: {
                saleType: saleType,
                houseType: houseType,
                area: area,
            ...
            success: function (result) {
                ...
                if (result.flag) {
                    $.ajaxFileUpload({
                        url: url + "/house/addPs",//用于文件上传的服务器端请求地址
